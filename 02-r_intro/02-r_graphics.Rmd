...
```{r echo = FALSE}
state_subset <- c("Alabama","Alaska", "Arizona", "Arkansas", "California", "Colorado")
library(dplyr)
fbiwide_subset <- fbiwide %>% filter(State %in% state_subset)
```

```{r, fig.width=10, fig.height=5, echo = TRUE}
ggplot(data = fbiwide_subset, aes(x = Burglary, y = Murder)) +
